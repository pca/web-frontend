...
const LoginPrompt = props => {
  ...
  const checkIfCanChangeRegion = user => {
    ...
    let canChange = false
    const dateUpdated = user.data.region_updated_at
      ? user.data.region_updated_at
      : user.data.created_at
    const yearToday = getYear(new Date())

    if (
      getYear(parseJSON(dateUpdated)) !== yearToday ||
      (user.data.region == null && statusOfTheUsersRequest === undefined)
    ) {
      canChange = true
    }
    ...
    statusOfTheUsersRequest === "Denied" ? (
      <React.Fragment>
        <p className="mt-1 text-sm leading-5">
          ...
          region. <br />
          You must e-mail us at <strong>pcaresultscommittee@gmail.com</strong>,
